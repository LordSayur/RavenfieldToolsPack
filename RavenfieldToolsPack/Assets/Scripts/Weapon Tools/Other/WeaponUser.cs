...
public class WeaponUser : MonoBehaviour {
	...
	void Start () {
		...
		SetWeapon(FindObjectOfType<Weapon>());
	...
	void SetWeapon(Weapon weapon) {
		...
		SetAimFov(weapon.configuration.aimFov);
	...
	void Update () {
		...
		this.aiming = Input.GetMouseButton(1) && !this.weapon.reloading && this.weapon.unholstered;
